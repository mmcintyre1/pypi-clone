...
def do_user_import(user_lookup: Dict[str, str]) -> Dict[str, User]:
    ...
    for (email, name) in tqdm(user_lookup.items()):
        session = db_session.create_session()
...
def do_load_files() -> List[dict]:
    data_path = pathlib.Path(__file__).parent.parent.parent / 'data' / 'pypi-top-100'
...
def init_db():
    ...
    rel_file = os.path.join('..', 'db', 'pypi_org.sqlite')
